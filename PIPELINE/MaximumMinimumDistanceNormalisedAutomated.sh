...
we loop over to generate networks for them all*)
(*its important to \
...
ErdosRenyiGraphsx100[x_] := Module[{graph = x},
  allmaxdists1 = {};
  allmaxdists2 = {};
  newmaxdists = {};
  For[m = 1, m <= 100, m++,
    Print["ER net number:", ToString[m], ""];
...
distribution*)
   ...
   (*return the max min dists*)
   Return[Mean[newmaxdists], Module]
  ]
...
For[ k = 1, k <= Length[indexlist], k++,
 ...
 maxmindistlist = {};
 (*Find the average of distances across the network in each frame. \
Remove Infinities as Thats the distance found between 2 connected \
components. Also remove 0s thats the distance between a node and \
itself. *)
